...
export { selectedMin, selectedMax }
...
class SortProducts extends Component {
    ...
    getItemList = () => [
        {
            name: i18n.t('Sort by - Newest to Oldest'),
            params: {
                sort_by: 'timestamp',
                sort_order: 'desc',
            },
        ...
        {
            name: i18n.t('Sort by - Lowest to Highest'),
            params: {
                sort_by: 'price',
                sort_order: 'asc',
            },
        ...
        {
            name: i18n.t('Sort by - Highest to Lowest'),
            params: {
                sort_by: 'price',
                sort_order: 'desc',
            },
        ...
        {
            name: i18n.t('Sort by - Most Popular'),
            params: {
                sort_by: 'popularity',
                sort_order: 'desc',
            },
    ...
    handleChangeFilter = () => {
        ...
        Object.keys(groupedFilters).forEach((key) => {
            ...
            if (filter_style === 'slider') {
                ...
                if (field_type === 'P') {
                    filtersIds.push(
                        `${filter_id}-${round(active.min, 2)}-${round(active.max, 2)}-${active.extra
                        }`,
                    );
                } else {
                    filtersIds.push(
    ...
    renderRange = (item) => {
        ...
        const activeFilter = selectedFilters.find(
         ...
         selectedMin = activeFilter ? activeFilter.min : Math.ceil(min);
         selectedMax = activeFilter ? activeFilter.max : Math.ceil(max);
        ...
        return (
            <View style={styles.pickerWrapper} key={feature_id}>
                <View style={styles.pickerOpenBtn}>
                    <Text style={styles.pickerOpenBtnText}>
                        {`${filter}: ${prefix || ''}${selectedMin}${suffix || ''} - ${prefix || ''
                            }${selectedMax}${suffix || ''}`}
                    </Text>
                </View>
                <View style={styles.priceRangeMarkerContainer}>
                    <Text style={styles.priceRangeMarkerText}>{`${prefix || ''
