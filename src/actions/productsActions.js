...
import Categories, { encryptbase } from "../screens/Categories"
...
export function fetchByCategory(
    ...
    // `sra_products&cid=${categoryId}&features_hash=1-${selectedMin}-${selectedMax}-INR_5-${encryptbase}`
    // `/categories/${categoryId}/sra_products`
    // `/categories/${categoryId}/sra_products&cid=${categoryId}&features_hash=1-${selectedMin}-${selectedMax}-INR_5-${encryptbase}`
    return (dispatch) => {
...
export function fetchByVendorlocation(
    categoryId,
    page = 1,
    companyId = false,
    advParams = {},
    ...
    return (dispatch) => {
        dispatch({ type: FETCH_PRODUCTS_REQUEST });
        return Api.get(`sra_products&cid=${categoryId}&features_hash=1-${selectedMin}-${selectedMax}-INR_5-${encryptbase}`, { params })
            .then((response) => {

                console.log("vendor_locationoutput:", response.data)
